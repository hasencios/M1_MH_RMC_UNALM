...
title: "Manipulando las salidas de los GCM"
teaching: 45
exercises: 135
questions:
- "¿Cómo manipulamos las salidas de los GCM?"
objectives:
- "Describir el procedimiento para la manipulación de las salidas de los GCM"
keypoints:
- "R nos ayuda a sistematizar el manejo de las salidas de los GCM."
source: Rmd
---
...
# Definimos las funciones que vamos a utilizar en la lección
# Función para estimar la media areal de GCM usando un shape de cuenca
getMean <- function(x,mask){
  x1 <- raster::crop(x,extent(mask)*1.5)
  x2 <- raster::raster(x1)
  res(x2) <- c(0.01,0.01)
  x2 <- raster::resample(x1,x2,method='ngb')
  x3 <- raster::mask(x2,mask)
...
## Seleccionamos el modelo y la variable de cada GCM

El objetivo es obtener la serie de tiempo media areal de cada una de nuestras unidades hidrográficas
...
Ahora vamos a hacer un listado, desde RStudio, de todo los GCM que hemos descargado

~~~
# Lista de los GCM descargados
dataset <- list.files('.',pattern = var)
...
~~~
[1] "tasmax_Amon_CanESM2_historical_r1i1p1_185001-200512.nc" "tasmax_Amon_CanESM2_rcp26_r1i1p1_200601-210012.nc"     
[3] "tasmax_Amon_CanESM2_rcp45_r1i1p1_200601-210012.nc"      "tasmax_Amon_CanESM2_rcp85_r1i1p1_200601-210012.nc"     
~~~
{: .output}

## Exploramos el GCM seleccionado

En esta parte vamos a hacer un plot para verificar que la referencia geográfica y las unidades sean las correctas

~~~
# Seleccionamos el archivo específico
wich <- 1

# Leemos el GCM y realizamos un plot
gcm <- brick(subset[wich])
plot(gcm[[1]])
~~~
{: .language-r}
...
<figure>
  <img src="../fig/11_GCM_tmax.png" style="width:100%">
</figure>

Podemos resaltar que la información descargada tiene una extensión global. Además, que la longitud está comprendida en el rango 0 a 360. Asimismo, los valores de temperatura están en grados Kelvin.

~~~
# Vamos a restar 180 a la longitud para tener la información en el rango -180 a 180
gcm2 <- rotate(gcm)
plot(gcm2[[2]])
~~~
{: .language-r}
...
<figure>
  <img src="../fig/11_GCM_tmax_corr.png" style="width:100%">
</figure>

## Ahora vamos a definir el vector de fechas

Como se trata de una serie de tiempo, es necesario definir un vector de fechas para asociar cada valor a un mes específico.

~~~
# Extraemos los años y meses del GCM descargado
name <- names(gcm2)
name <- substr(name, 2, nchar(name))
(years <- substr(name, 1, 4))
(month <- substr(name, 6, 7))

# Vector de fechas
ini <- as.Date(paste0(as.character(years[1]),'/',as.character(month[1]),'/','15'),format="%Y/%m/%d")
fin <- as.Date(paste0(as.character(years[length(years)]),'/',as.character(month[length(month)]),'/','15'),format="%Y/%m/%d")
fechas <- seq(ini,fin,by="month")

# Determinamos el numero de días en cada mes 
num_days <- days_in_month(fechas)
~~~
{: .language-r}

## Leyendo las unidades hidrográficas de interés

En este paso vamos a leer los `shapefiles` de nuestras subcuencas para estimar los medios areales
...
## Ahora vamos a estraer las series de tiempo de cada subcuenca de interés
~~~
# Vamos a medir el tiempo de calculo
start_time <- Sys.time()

# Se va a extraer la serie de tiempo en paralelo 
plan(multisession)

# Vamos a hacer un loop a través de cada unidad hidrográfica
for(i in 1:length(mask1)){
  # Seleccionamos la subcuenca de interés
  w <- i - 1
  mask2 <- subset(mask1, DN %in% c(as.character(w)))
  
  # Extraemos la serie de tiempo en paralelo
  TS <- 1:nlayers(gcm2) %>% 
    future_map(~ getMean(gcm2[[.x]], mask2)) %>%
    future_map_dfr(~ as.data.frame(.))
  
  # Realizamos el cambio de unidades
  if(var == 'pr'){
    TS <- TS*num_days*60*60*24
  }else{
    TS <- TS - 273.15
  }
  
  # Creando dataframe y guardando la serie de tiempo
  df <- data.frame(fechas,TS)
  colnames(df) <- c('dates','var')
  x <- zoo(df$var,df$dates)
